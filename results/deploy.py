...
def main():
    ...
    for i in range(6, NUMBER_TESTS+3):
        ...
        instantiate_ns(ns_name=CLUSTER_NAME, vnf_name_master='cluster_vnf', nsd_name='k8s_cluster_nsd', ns_config_file=PATH_OSM_NS_CONFIG_FILE, results_path=results_path)
...
#
        ...
        while True:
            ...
            for ns_instance in response.json():
                print(ns_instance)
                if stop[CLUSTER_NAME] and CLUSTER_NAME == ns_instance['name']:
                    nsi_ids.append(ns_instance['_id'])
