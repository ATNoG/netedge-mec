...
def instantiate_ns(ns_name, nsd_name, ns_config_file):
    # instantiate cluster
    ...
    output = subprocess.run(shlex.split(
        ...
        --vnf_name osm_vnf --vdu_id controller --action_name get-k8s-controller-info --wait"""
    ...
    output = subprocess.run(shlex.split(
        f"""osm --hostname {IP_ADDR} --user {USER_MAIN} --password '{PASSWORD_MAIN}' --project {PROJECT_MAIN} ns-action {ns_name} 
        --vnf_name osm_vnf --vdu_id worker --vdu_count 0 --action_name join-k8s-workers --params_file {PATH_JOIN_PARAMS_FILE} --wait"""
    ), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    print(output)
...
def clean_environment(ns_osm_name: str, ns_main_name: str):
    print(f"\n\n\n{time.time()} - Delete OSM NS {ns_osm_name}\n")
    output = subprocess.run(shlex.split(
    ...
    print(f"\n\n\n{time.time()} - Delete OSM NS {ns_main_name}\n")
    output = subprocess.run(shlex.split(
        f"""osm --hostname {IP_ADDR} --user {USER_MAIN} --password '{PASSWORD_MAIN}' --project {PROJECT_MAIN} ns-delete 
        {ns_main_name} --wait"""
    ))
...
def main():
    ...
    instantiate_ns(ns_name=CLUSTER_FOR_OSM_NAME, nsd_name='k8s_osm_cluster_nsd', ns_config_file=PATH_OSM_NS_CONFIG_FILE)

    # obtain the IP addr of the first NS master
    print(f"\n\n\n{time.time()} - Obtaining master's IP addr\n")
    output = subprocess.run(shlex.split(
        f"""osm --hostname {IP_ADDR} --user {USER_MAIN} --password '{PASSWORD_MAIN}' --project {PROJECT_MAIN} vnf-list --ns cluster"""
    ), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    print(output)
    ...
    instantiate_ns(ns_name=CHARMED_OSM_NAME, nsd_name='k8s_cluster_nsd', ns_config_file=PATH_CLUSTER_NS_CONFIG_FILE)
